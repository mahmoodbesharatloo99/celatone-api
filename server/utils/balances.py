...
def get_hive_balance_legacy(chain, network, account_address):
    output_balance = []
...
def get_native_balances_legacy(endpoint, chain, network, account_address):
    balances = (
        requests.get(
...
def get_native_balances(chain, network, address):
    endpoint = get_network_data(chain, network, "lcd")
    balances = (
        requests.get(
            f"{endpoint}/cosmos/bank/v1beta1/balances/{address}?pagination.limit=500"
        )
        .json()
        .get("balances", [])
    )
